...
public class ExceptionHandlerGlobal extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ErrorResponce> userExceptionHandler(WhatsappException we, WebRequest wb, Exception e) {
		ErrorResponce error = ErrorResponce.builder().errorCode(1).status(HttpStatus.BAD_REQUEST.value())
				...
				.errorDetails(we.getMessage()).build();
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		WhatsappErrorResponce whatsappVException = new WhatsappErrorResponce(HttpStatus.UNPROCESSABLE_ENTITY.value(),
				error.get(0).getDefaultMessage());
		return new ResponseEntity<>(whatsappVException, HttpStatus.UNPROCESSABLE_ENTITY);
	}

	@Override
	protected ResponseEntity<Object> handleTypeMismatch(TypeMismatchException ex, HttpHeaders headers,
			HttpStatus status, WebRequest request) {
		WhatsappErrorResponce resp = new WhatsappErrorResponce(HttpStatus.UNPROCESSABLE_ENTITY.value(),
				ex.getMessage());
...
//		IllegalStateException.class,AccessDeniedException.class })
//	public ResponseEntity<Object> serverErrorHandler(WebRequest wb,RuntimeException ex){
